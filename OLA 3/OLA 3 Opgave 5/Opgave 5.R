...
library(stringr)
library(tidyverse)
nøgle <- c("d5a4ceb4-12b0-4c59-8454-f5480b852a08")
tid <- c("2023-10-19T00:00:00Z/2023-10-22T00:00:00Z")
baseURL <- c("https://dmigw.govcloud.dk/v2/metObs/collections/observation/items?parameterId=")

anholtWindspeed <- GET(url=paste0(baseURL, "wind_speed&datetime=", tid,"&stationId=06079&api-key=", nøgle))
anholtWinddir <- GET(url=paste0(baseURL, "wind_dir&datetime=", tid,"&stationId=06079&api-key=", nøgle))
aarhusWindspeed <- GET(url=paste0(baseURL, "wind_speed&datetime=", tid,"&stationId=06074&api-key=", nøgle))
aarhusWinddir <- GET(url=paste0(baseURL, "wind_dir&datetime=", tid,"&stationId=06074&api-key=", nøgle))
